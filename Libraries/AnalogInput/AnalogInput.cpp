...
AnalogInput::AnalogInput(String id)
{
	_id = String(id);
	_pin = NOT_INIT;
	_TagType = ANALOG_INP;
	_PV_Hi = NOT_INIT_VAL;
	_PV_Lo = NOT_INIT_VAL;
	_Raw_Hi = NOT_INIT_VAL;
	_Raw_Lo = NOT_INIT_VAL;
	_CalcType = PV_AUTO;
}

unsigned char AnalogInput::Init()
{
	_Err = NO_ERR;
	if (_pin == NOT_INIT) _Err = _Err + PIN_ERR;
	if (_TagType == NOT_INIT) _Err = _Err + TYPE_ERR;
	if (_PV_Hi == NOT_INIT_VAL) _Err = _Err + PV_HI_ERR;
	if (_PV_Lo == NOT_INIT_VAL) _Err = _Err + PV_LO_ERR;
	if (_Raw_Hi == NOT_INIT_VAL) _Err = _Err + RAW_HI_ERR;
	if (_Raw_Lo == NOT_INIT_VAL) _Err = _Err + RAW_LO_ERR;
	return _Err;
}

String AnalogInput::getId()
{
	return _id;
}
...
void AnalogInput::setPin(unsigned char pin)
{
	_pin = pin;
}
unsigned char AnalogInput::getPin()
{
	return _pin;
}
...
void AnalogInput::setTagType(unsigned char val)
{
	_TagType = val;
}
unsigned char AnalogInput::getTagType()
{
	return _TagType;
}

unsigned char AnalogInput::getErr()
{
	return _Err;
}
